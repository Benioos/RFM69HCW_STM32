...
#define AES_DEBUG_ENABLED // Assure-toi que c'est bien défini AVANT le bloc ifdef

#ifdef AES_DEBUG_ENABLED
    // Le ## avant __VA_ARGS__ est magique : il supprime la virgule si la liste d'arguments est vide
    #define AES_printf(color, prefix, fmt, ...) \
        printf(color prefix " : " fmt, ##__VA_ARGS__)

    #define AES_printfs(fmt, ...) printf(fmt, ##__VA_ARGS__)
#else
    #define AES_printf(color, prefix, fmt, ...)
    #define AES_printfs(fmt, ...)
#endif

#ifdef AES_DEBUG_ENABLED
    #define AES_Delay(ms) HAL_Delay(ms)
#else
    #define AES_Delay(ms)
#endif
